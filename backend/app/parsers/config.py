...
class ParserConfigProvider:
    """Reads parser configuration from the centralized settings layer.

    All parser behavior is driven by values from the ``parsing``
    settings namespace (env prefix: ``PARSING_``). No values are
    hardcoded in parser implementations.
    """
