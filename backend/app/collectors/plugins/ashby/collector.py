...
class AshbyCollector(BaseCollector):
    ...
    async def collect(self, query: CollectorQuery) -> CollectorResult:
        ...
        url = f"{ASHBY_API_BASE}/{board_token}?includeCompensation=true"
        ...
        try:
            ...
            errors.append(
                ErrorReport(
                    ...
                    error_message="Invalid JSON from Ashby API",
                    recoverable=False,
                )
            )
            return self._build_result(query, all_jobs, errors, pages_collected)
        ...
        if not isinstance(data, dict):
            errors.append(
                ErrorReport(
                    error_type="ParsingError",
                    error_message="Unexpected response format from Ashby API",
                    recoverable=False,
                )
            )
            return self._build_result(query, all_jobs, errors, pages_collected)
        ...
        if not isinstance(jobs_raw, list):
            errors.append(
                ErrorReport(
                    error_type="ParsingError",
                    error_message="Jobs field is not a list in Ashby API response",
                    recoverable=False,
                )
            )
            return self._build_result(query, all_jobs, errors, pages_collected)
    ...
    def _build_result(
        ...
        raw_data = {"jobs": all_jobs, "board_token": self._get_board_token(query)}
    ...
    async def _fetch_page(self, url: str) -> httpx.Response:
        ...
        try:
            ...
            raise NetworkError(
                f"Request to Ashby API timed out",
                source=self.source_id,
                original=e,
            )
        except httpx.RequestError as e:
            raise NetworkError(
                f"Request to Ashby API failed: {e}",
                source=self.source_id,
                original=e,
            )
        ...
        if response.status_code == 429:
            ...
            raise RateLimitError(
                "Rate limited by Ashby API",
                retry_after=retry_after,
                source=self.source_id,
            )
        if response.status_code == 404:
            raise NetworkError(
                f"Ashby board '{url.split('/')[-1].replace('?includeCompensation=true', '')}' not found (HTTP 404)",
                status_code=404,
                source=self.source_id,
            )
        if response.status_code != 200:
            raise NetworkError(
                f"Ashby API returned HTTP {response.status_code}",
        ...
        try:
            ...
            raise ParsingError(
                "Invalid JSON response from Ashby API",
                source=self.source_id,
                original=e,
            )
    ...
    async def normalize(self, raw_data: Any) -> list[JobData]:
        ...
        for job in jobs_raw:
            try:
                job_data = self._normalize_single(job, board_token)
    ...
    def _normalize_single(
        ...
        company_name = job.get("companyName", "") or board_token.title()
        company = CompanyData(name=company_name)

        locations_raw = job.get("locations", [])
        location_str = locations_raw[0] if locations_raw else ""
        location = self._parse_location(location_str)

        employment_type_raw = job.get("type", "")
        employment_type = employment_type_raw.lower() if employment_type_raw else None
        ...
        if published_at_raw:
            try:
                posted_at = datetime.fromisoformat(published_at_raw)
        ...
        salary = self._parse_compensation(job.get("compensationSummary"))
        ...
        return JobData(
            ...
            skills=[department] if department else [],
            raw_data=job,
        )
    ...
    def _parse_location(self, location_str: str) -> LocationData:
        ...
        if is_remote:
            ...
            if " - " in location_str:
                country = location_str.split(" - ")[-1].strip() if " - " in location_str else None
            elif len(parts) >= 2:
    ...
    def _parse_compensation(self, summary: Optional[str]) -> Optional[SalaryData]:
        if not summary:
            return None
        ...
        interval = "yearly"
        salary_text_lower = summary.lower()
        if "/hr" in salary_text_lower or "per hour" in salary_text_lower:
            interval = "hourly"
        elif "/mo" in salary_text_lower or "month" in salary_text_lower:
            interval = "monthly"
